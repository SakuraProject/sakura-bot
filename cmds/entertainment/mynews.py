...
class mynews(commands.Cog): 
    ...
    async def post(self,ctx):
        ...
        **使いかた：**
        ...
        try:
            ...
            async with self.bot.session.post("http://ysmsrv.wjg.jp/news/sendbydiscord.php",data=req) as resp:
                rpt = await resp.text()
                await ctx.send(rpt)
        except SyntaxError:
    ...
    async def day(self,ctx,day):
        ...
        **使いかた：**
        ...
        ELang default
        """
        async with self.bot.session.get("https://ysmsrv.wjg.jp/news/timebydiscord.php?input_date="+urllib.parse.quote_plus(day, encoding='utf-8')) as resp:
            rpt = await resp.text()
            if rpt=="[]":
...
async def getusername(userid):
    async with client.session.get("https://ysmsrv.wjg.jp/news/"+userid) as resp:
        rpt = await resp.text()
        return rpt
    ...
    async def callback(self, interaction: discord.Interaction):
        for item in self.its:
            if item["title"] == self.values[0]:
                ebd = discord.Embed(title=item["title"], description=item["text"], color=client.Color)
                ebd.add_field(name="投稿者", value=(await getusername(item["user"])))
