...
class giveaway(commands.Cog):
                ...
                for gk in self.gavs:
                    ...
                    if endgv<now:
                        ...
                        for i in range(len(gaway['cid'])):
                            try:
                                channel = self.bot.get_channel(int(gaway['cid'][str(i)]))
                                if channel!=None:
                                    try:
                            ...
                            if channel!=None:
                                if new_message!=None:
                                    ...
                                    try:
                                        userstemp.pop(users.index(self.bot.user))
                                    except:
                                        str('poperror')
                        ...
                        if len(users)==0:
                            ...
                            for i in range(win-1):
                                wusgiv=random.choice(users)
                                wusgivt=wusgiv.name+'#'+wusgiv.discriminator
                                winner = winner+','+wusgiv.mention+'('+wusgivt+')'
                            for i in range(len(gaway['cid'])):
                                winning_announcement = discord.Embed(color = 0xff2424)
                                winning_announcement.set_author(name = f'THE GIVEAWAY HAS ENDED!', icon_url= 'https://i.imgur.com/DDric14.png')
                                winning_announcement.add_field(name = f'🎉 景品: {prize}', value = f'🥳 **勝者**: {winner}\n 🎫  **参加者数**: {len(users)}', inline = False)
                                winning_announcement.set_footer(text = 'Thanks for entering!')
                                channelt = self.bot.get_channel(int(gaway['cid'][str(i)]))
                                if channelt!=None:
    ...
    async def join(self,ctx,code):
        ...
        try:
            ...
            endstr=datetime.datetime.fromtimestamp(int(jself.gavs['end'])).strftime('%Y/%m/%d %H:%M')
            give = discord.Embed(color = 0x2ecc71)
            give.set_author(name = f'GIVEAWAY TIME!', icon_url = 'https://i.imgur.com/VaX0pfM.png')
            give.add_field(name= f'{author}が開催した抽選: {prize}!', value = f'🎉を押して参加!\n 終了時刻{endstr}!', inline = False)
            my_message = await channel.send(embed = give)
            await my_message.add_reaction("🎉")
            self.gavs[code]['cid'][str(len(self.gavs[code]['cid']))]=channel.id
            self.gavs[code]['mid'][str(len(self.gavs[code]['mid']))]=my_message.id
            await self.save(code)
        except KeyError:
            await ctx.send(f'招待コードが無効です')
    @giveaway.command()
    async def create(self,ctx):
        giveaway_questions = ['どのチャンネルで開催する?', '景品は何にする?', 'Giveawayは何秒間開催する？','当選者は何人にする？',]
        giveaway_answers = []
        def check(m):
            return m.author == ctx.author and m.channel == ctx.channel
        for question in giveaway_questions:
        ...
        give = discord.Embed(color = 0x2ecc71)
        give.set_author(name = f'GIVEAWAY TIME!', icon_url = 'https://i.imgur.com/VaX0pfM.png')
        give.add_field(name= f'{ctx.author.name}が開催した抽選: {prize}!', value = f'🎉を押して参加!\n 終了時刻{round(times/60/60, 2)}時間!', inline = False)
