    ...
    async def on_tweet(self, tweet):
        ...
        if tweet.referenced_tweets != None:
            ...
            if tweet.referenced_tweets[0].type == "replied_to":
                status.text = twiname + " Reply to [This Tweet](https://twitter.com/" + self.api.get_user(id=self.api.get_tweet(tweet.referenced_tweets[0].id,user_fields="username",expansions="author_id").data.author_id,user_fields="username,profile_image_url").data.username + "/status/" + str(tweet.referenced_tweets[0].id) + ")\n" + tweet.text
        async with self.bot.pool.acquire() as conn:
            async with conn.cursor() as cur:
